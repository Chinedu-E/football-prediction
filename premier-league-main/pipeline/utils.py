...
def get_past_h2h(data: pd.DataFrame, home_team: str, away_team: str, main_index: int):
    """Gets the results of the past 3 head-to-head of both teams.
        ...
        Returns:
            A list of integers, representing the results of the past 3 h2h matches. 0 indicates an away win, 1 for a draw and 2 for a home win.
    ...
    if found < 3:
        past_results = [1] * (3 - found)
        for i in range(found):
            past_match_index = indexes[i]
            past_results.append(get_match_result(data, home_team, away_team, past_match_index))
    else:
        past_results = []
        for i in range(found - 3, found):
            past_match_index = indexes[i]
            past_results.append(get_match_result(data, home_team, away_team, past_match_index))
    return past_results


def get_past_h2h_(data: pd.DataFrame, home_team: str,
...
def get_past_form(data: pd.DataFrame, team: str,
    ...
    elif len(match_indexes) == 2:
        past_result1 = get_match_result(data, team, team, match_indexes[-1])
        past_result2 = get_match_result(data, team, team, match_indexes[-2])
    elif len(match_indexes) == 3:
        past_result1 = get_match_result(data, team, team, match_indexes[-1])
        past_result2 = get_match_result(data, team, team, match_indexes[-2])
        past_result3 = get_match_result(data, team, team, match_indexes[-3])
    elif len(match_indexes) == 4:
        past_result1 = get_match_result(data, team, team, match_indexes[-1])
        past_result2 = get_match_result(data, team, team, match_indexes[-2])
        past_result3 = get_match_result(data, team, team, match_indexes[-3])
        past_result4 = get_match_result(data, team, team, match_indexes[-4])
    elif len(match_indexes) >= 5:
        past_result1 = get_match_result(data, team, team, match_indexes[-1])
        past_result2 = get_match_result(data, team, team, match_indexes[-2])
        past_result3 = get_match_result(data, team, team, match_indexes[-3])
        past_result4 = get_match_result(data, team, team, match_indexes[-4])
        past_result5 = get_match_result(data, team, team, match_indexes[-5])
    return past_result1, past_result2, past_result3, past_result4, past_result5
